...
- 同一日期可重複歸檔，後一次會以目前 JSON 完整快照更新原資料。
...
4. 同一天再次歸檔時，先清除該日舊場次，再依目前 JSON 重建完整快照。
5. 歷史統計只讀取已成功歸檔的 MariaDB 資料。
...
外鍵使用 `ON DELETE CASCADE`，同一天重新歸檔時可安全重建場次與崗位資料。
